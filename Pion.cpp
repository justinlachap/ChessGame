...
{
	// si n'a pas boug�, il peut avancer de 2 cases vers l'avant
	...
	if (position_.first < tailleEchiquierMax && (e.cases[position_.first + uneCase][position_.second + uneCase] != caseVide && e.cases[position_.first + uneCase][position_.second + uneCase]->obtenirCouleur() != estBlanc_))
	...
	if (position_.first > tailleEchiquierMin && (e.cases[position_.first - uneCase][position_.second + uneCase] != caseVide && e.cases[position_.first - uneCase][position_.second + uneCase]->obtenirCouleur() != estBlanc_))
		mouvementsDisponibles_.push_back(std::pair(position_.first - uneCase, position_.second + uneCase));
}

void Pion::mouvementsPionNoir(Echiquier &e)
{
	// si pion noir n'a pas boug�, il peut avancer de 2 cases vers l'avant
	if ((position_.second == tailleEchiquierMax - 1) && (e.cases[position_.first][position_.second - avancerDeuxCases] == caseVide) && (e.cases[position_.first][position_.second - avancerUneCase] == caseVide))
		mouvementsDisponibles_.push_back(std::pair(position_.first, position_.second - avancerDeuxCases));
